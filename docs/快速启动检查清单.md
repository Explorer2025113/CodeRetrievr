# CodeRetrievr 快速启动检查清单

使用此清单快速检查您的部署状态。

## ✅ 部署前检查

### 1. 系统软件
- [ ] Python 3.9+ 已安装
  ```bash
  python --version
  ```
- [ ] Docker Desktop 已安装并运行
  ```bash
  docker --version
  docker info
  ```
- [ ] Git 已安装
  ```bash
  git --version
  ```

### 2. 项目代码
- [ ] 项目已克隆到本地
  ```bash
  cd CodeRetrievr
  ls
  ```
- [ ] 项目目录结构完整
  - [ ] `app/` 目录存在
  - [ ] `scripts/` 目录存在
  - [ ] `docker-compose.yml` 文件存在
  - [ ] `requirements.txt` 文件存在

### 3. Python 环境（推荐使用 Anaconda）
- [ ] Conda 环境已创建
  ```bash
  # 创建环境
  conda create -n coderetrievr python=3.9 -y
  
  # 激活环境
  conda activate coderetrievr
  ```
- [ ] Python 依赖已安装
  ```bash
  # 确保已激活环境
  conda activate coderetrievr
  
  # 检查依赖
  pip list | grep fastapi
  pip list | grep pymilvus
  ```
- [ ] 或使用 venv（不推荐）
  ```bash
  source venv/bin/activate  # Linux/macOS
  venv\Scripts\activate     # Windows
  ```

### 4. 环境变量配置
- [ ] `.env` 文件已创建
  ```bash
  ls .env
  ```
- [ ] 必需环境变量已配置
  - [ ] `LLM_API_KEY` (DeepSeek/OpenAI API密钥)
  - [ ] `GITHUB_TOKEN` (GitHub Token)
  - [ ] `NEO4J_PASSWORD` (Neo4j密码，需与docker-compose.yml一致)

### 5. Docker 服务
- [ ] Docker 服务已启动
  ```bash
  docker-compose ps
  ```
- [ ] 所有服务状态为 "healthy" 或 "Up"
  - [ ] Milvus (code-retrievr-milvus)
  - [ ] Neo4j (code-retrievr-neo4j)
  - [ ] etcd (code-retrievr-etcd)
  - [ ] MinIO (code-retrievr-minio)

## ✅ 验证步骤

### 1. 运行环境检查脚本
```bash
python scripts/check_environment.py
```

**期望输出**：
```
✅ Python版本: 3.9.x
✅ FastAPI
✅ PyMilvus
✅ Neo4j
✅ 环境变量配置完整
✅ Docker服务正在运行
✅ Milvus矢量数据库 (code-retrievr-milvus)
✅ Neo4j知识图谱 (code-retrievr-neo4j)
🎉 环境配置检查全部通过！
```

### 2. 验证 Milvus 连接
```bash
curl http://localhost:9091/healthz
```

**期望输出**：`{"status":"ok"}` 或 HTTP 200

### 3. 验证 Neo4j 连接
```bash
# 访问 http://localhost:7474
# 或使用命令行
docker exec code-retrievr-neo4j cypher-shell -u neo4j -p your_actual_password_123 "RETURN 1"
```

**期望输出**：返回 `1`

### 4. 测试后端服务
```bash
python -m uvicorn app.main:app --reload --host 0.0.0.0 --port 8000
```

**期望输出**：
```
INFO:     Uvicorn running on http://0.0.0.0:8000 (Press CTRL+C to quit)
INFO:     Started reloader process
INFO:     Started server process
INFO:     Waiting for application startup.
INFO:     Application startup complete.
```

访问 http://localhost:8000/docs 应该能看到 API 文档。

## ❌ 常见问题快速解决

### 问题：Docker 服务未启动
```bash
# 启动所有服务
docker-compose up -d

# 等待服务启动（约1-2分钟）
docker-compose ps

# 查看日志
docker-compose logs
```

### 问题：Neo4j 连接失败
```bash
# 检查容器状态
docker-compose ps neo4j

# 如果容器未运行，启动它
docker-compose up -d neo4j

# 等待容器启动（约30秒）
docker-compose ps neo4j  # 等待状态变为 "healthy"
```

### 问题：环境变量未加载
```bash
# 检查 .env 文件是否存在
ls .env

# 如果不存在，从模板创建
cp env .env  # Linux/macOS
copy env .env  # Windows

# 编辑 .env 文件，填入必需的配置
```

### 问题：Python 依赖安装失败
```bash
# 升级 pip
python -m pip install --upgrade pip

# 使用国内镜像源
pip install -r requirements.txt -i https://pypi.tuna.tsinghua.edu.cn/simple

# 单独安装 PyTorch（如果失败）
pip install torch --index-url https://download.pytorch.org/whl/cpu
```

## 🚀 下一步

环境配置完成后，您可以：

1. **采集代码**：
   ```bash
   python scripts/collect_code.py tiangolo/fastapi --language python
   ```

2. **向量化代码**：
   ```bash
   python scripts/vectorize_code.py data/code_snippets/your_file.json
   ```

3. **启动 API 服务**：
   ```bash
   python -m uvicorn app.main:app --reload
   ```

4. **访问 API 文档**：
   - 打开浏览器访问 http://localhost:8000/docs

## 📚 相关文档

- [新机器部署指南](./新机器部署指南.md) - 完整部署步骤
- [环境配置指南](./环境配置指南.md) - 详细配置说明
- [阶段2代码采集指南](./阶段2代码采集指南.md) - 代码采集步骤
- [阶段3向量化指南](./阶段3向量化指南.md) - 向量化步骤

---

**最后更新**：2024年11月

